...
class Contenedor {
    ...
    deleteById(id) {
        try {
            productArray = JSON.parse(fs.readFileSync(this.nombreDelArchivo, "utf8")).filter((product) => product.id != id);
            fs.writeFileSync(this.nombreDelArchivo, JSON.stringify(productArray, null, 2));
        }
        catch (error) {
            console.log(error);
    ...
    async deleteAll() {
        let emptyContent = {};
        await fs.promises.writeFile(this.nombreDelArchivo, JSON.stringify(emptyContent));
    }
}
...
// Productos.getById(3).then(val => console.log(val));

// Borrar el segundo producto y mostrar los resultados.

// Productos.deleteById(2);
// console.log(fs.readFileSync(Productos.nombreDelArchivo, "utf8"));

// Borrar todos los productos guardados y mostrar el contenido del archivo luego del proceso de borrado.
